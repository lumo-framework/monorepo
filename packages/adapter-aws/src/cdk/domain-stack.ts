...
import { HostedZone, IHostedZone, RecordType, ARecord, RecordTarget } from 'aws-cdk-lib/aws-route53';
...
export class DomainStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: DomainStackProps) {
    ...
    if ('create' in domainConfig.certificate && domainConfig.certificate.create) {
      // Create a new ACM certificate using DNS validation
      certificate = new Certificate(this, 'Certificate', {
        domainName,
        validation: domainType === 'external' 
      ...
      certificate = Certificate.fromCertificateArn(this, 'ImportedCertificate', domainConfig.certificate.arn);
  ...
  private validateDomainConfig(domainConfig: any): void {
    ...
    if (domainName.length > 253) {
      throw new Error(`Domain name too long: ${domainName} (max 253 characters)`);
    }
    
    // Validate domain type
    const validTypes = ['subdomain', 'hosted-zone', 'external'];
    if (!validTypes.includes(domainType)) {
      throw new Error(`Invalid domain type: ${domainType}. Must be one of: ${validTypes.join(', ')}`);
    }
    
    // Validate certificate configuration
    if (certificate) {
      if ('create' in certificate && 'arn' in certificate) {
        throw new Error('Certificate configuration cannot have both "create" and "arn" properties');
      }
      
      if ('arn' in certificate && (!certificate.arn || typeof certificate.arn !== 'string')) {
    ...
    if (domainType === 'hosted-zone' && parts.length > 2) {
      console.warn(`Warning: Using hosted-zone type for subdomain ${domainName}. Consider using type: 'subdomain' for better isolation.`);
    }
    
    if (domainType === 'subdomain' && parts.length <= 2) {
      console.warn(`Warning: Using subdomain type for root domain ${domainName}. Consider using type: 'hosted-zone' to manage the entire domain.`);
    }
  }
