...
import { buildRequestFromApiGateway, isApiGatewayProxyEvent } from './aws-request-builder.js';
...
describe('AWS Request Builder', () => {
    describe('buildRequestFromApiGateway', () => {
        ...
        test('should parse query parameters correctly', () => {
            ...
            assert.equal(request.url, 'https://api.example.com/search?q=javascript&page=2&sort=date');
            assert.deepEqual(request.query, {
                q: 'javascript',
                page: '2',
                sort: 'date'
        ...
        test('should handle different protocols from X-Forwarded-Proto', () => {
            ...
            assert.equal(buildRequestFromApiGateway(httpEvent).url, 'http://api.example.com/test');
            assert.equal(buildRequestFromApiGateway(httpsEvent).url, 'https://api.example.com/test');
        });
