...
import { createRequest, parseCookies, parseQuery } from '@tsc-run/core/dist/http/request.js';

/**
 * Builds a tsc-run Request object from an AWS API Gateway proxy event
 */
export const buildRequestFromApiGateway = (event: APIGatewayProxyEvent): http.Request => {
 ...
 */
export const isApiGatewayProxyEvent = (event: unknown): event is APIGatewayProxyEvent => {
    return (
        typeof event === 'object' &&
        event !== null &&
        'httpMethod' in event &&
