...
export function formatDeploymentOutput(result: any): string {
  ...
  if (result.customDomainUrl) {
    lines.push(`📡 \x1b[1mCustom Domain:\x1b[0m \x1b[36m${result.customDomainUrl}\x1b[0m`);
    if (result.apiGatewayUrl) {
      lines.push(`🔗 \x1b[1mAPI Gateway:\x1b[0m \x1b[90m${result.apiGatewayUrl}\x1b[0m`);
    }
  } else if (result.apiGatewayUrl) {
    lines.push(`📡 \x1b[1mAPI URL:\x1b[0m \x1b[36m${result.apiGatewayUrl}\x1b[0m`);
  } else {
    const displayUrl = result.url === 'URL not found' ? '\x1b[33mCheck CloudFormation outputs for API Gateway URL\x1b[0m' : result.url;
  ...
  if (result.domain) {
    ...
    lines.push(`   \x1b[1mDomain:\x1b[0m \x1b[36mhttps://${result.domain.name}\x1b[0m`);
    ...
    if (result.domain.setupInstructions) {
      ...
      if (result.domain.type === 'subdomain') {
        ...
        lines.push('   Check your email for AWS certificate validation messages');
        lines.push('   \x1b[33m💡 Validate the certificate first, then set up DNS\x1b[0m');
      }
      ...
      if (result.domain.cnameTarget) {
        ...
        lines.push('   \x1b[33m💡 Create this CNAME record in your external DNS provider\x1b[0m');
      }
    }
  }
