...
export const newCommand: CommandModule<{}, NewCommandArgs> = {
  ...
  handler: async (argv) => {
    ...
    try {
      console.log(`🚀 \x1b[1mCreating new tsc-run project: \x1b[36m${projectName}\x1b[0m\n`);

      // Check if directory already exists
      try {
        await fs.access(projectName);
        console.error(`❌ Directory \x1b[31m${projectName}\x1b[0m already exists!`);
        process.exit(1);
      } catch {
      ...
      execSync(`git clone https://github.com/tsc-run/tsc.run.git "${projectName}"`, {
        stdio: 'inherit',
      });

      // Remove .git directory from the cloned template
      console.log('🧹 Cleaning up template...');
      await fs.rm(path.join(projectName, '.git'), { recursive: true, force: true });
