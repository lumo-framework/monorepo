        ...
        try {
            ...
            if (methodRoutes.length > 0) {
                ...
                for (const [index, methodRoute] of methodRoutes.entries()) {
                    ...
                    const routeName = method === 'ALL' ? route : `${route}-${method.toLowerCase()}`;
                    
                    process.stdout.write(`   ${index + 1}/${methodRoutes.length} \x1b[33m${routeName}\x1b[0m `);
            ...
            if (subscribers.length > 0) {
                ...
                for (const [index, subscriber] of subscribers.entries()) {
                    ...
                    process.stdout.write(`   ${index + 1}/${subscribers.length} \x1b[33m${name}\x1b[0m `);
            ...
            console.log(`📦 Generated ${methodRoutes.length + subscribers.length} Lambda functions in \x1b[36mdist/lambdas/\x1b[0m\n`);
...
async function generateLambdaWrapper(routeFile: string, method: string): Promise<string> {
...
async function generateSubscriberWrapper(subscriberFile: string): Promise<string> {
    const wrapperContent = `
