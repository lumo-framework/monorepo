...
type ResponseWithoutFunctions = Omit<
  Response,
  'json' | 'html' | 'redirect' | 'cookie' | 'attachment' | 'status' | 'header'
>;
...
const json =
  (response: ResponseWithoutFunctions) =>
  (data: string | Record<string, unknown>): Response => {
...
const redirect =
  (response: ResponseWithoutFunctions) =>
  (url: string, statusCode: number = 302): Response => {
