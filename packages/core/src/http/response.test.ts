...
describe('Response', () => {
  ...
  describe('json method', () => {
    ...
    test('should be chainable', () => {
      const res = response().json({ data: 'test' }).header('X-Custom', 'value');
  ...
  describe('html method', () => {
    ...
    test('should be chainable', () => {
      const res = response().html('<h1>Title</h1>').header('X-Page', 'home');
  ...
  describe('cookie method', () => {
    ...
    test('should set cookie with options', () => {
      ...
      const expectedCookie =
        'secure-session=xyz789; Max-Age=3600; Path=/; Secure; HttpOnly; SameSite=strict';
    ...
    test('should set cookie with expires option', () => {
      ...
      assert.equal(
        ...
        'temp=value; Expires=Mon, 01 Jan 2024 00:00:00 GMT'
      );
    });

    test('should set cookie with domain option', () => {
      const res = response().cookie('cross-domain', 'value', {
        domain: '.example.com',
      });

      assert.equal(
        res.headers['Set-Cookie'],
        'cross-domain=value; Domain=.example.com'
      );
    });
  ...
  describe('cookie formatting edge cases', () => {
    test('should handle all cookie options together', () => {
      ...
      const expectedCookie =
        'admin-session=secret; Max-Age=86400; Expires=Tue, 31 Dec 2024 23:59:59 GMT; Domain=.example.com; Path=/admin; Secure; HttpOnly; SameSite=lax';
      assert.equal(res.headers['Set-Cookie'], expectedCookie);
    });

    test('should handle sameSite values correctly', () => {
      const strict = response().cookie('strict', 'value', {
        sameSite: 'strict',
      });
